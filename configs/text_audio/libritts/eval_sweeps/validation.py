...
def get_config():
    ...
    cfg.train.textaudio.num_samples = 256
    cfg.train.textaudio.whisper_model = 'openai/whisper-large'

    cfg.train.textaudio.sampling_sweep = config_dict.ConfigDict()
    cfg.train.textaudio.sampling_sweep.specs = []

    # s_churn = gamma_target * num_intervals (same treatment as
    # configs/owt/eval_1M.py). sc_refresh_mode="carry" (1 NFE/step) means
    # num_intervals resolves to target_nfe - 1.
    from evaluation.nfe import steps_for_target_nfe

    def add_textaudio_spec(sampler_name, target_nfes, stochastic_enabled=False, gamma_targets=None, guidance_scales=None):
    ...
    add_textaudio_spec(
        'ddim_entropic', target_nfes=[256], stochastic_enabled=True,
        gamma_targets=[0.11, 0.13, 0.15, 0.17, 0.19, 0.22, 0.25, 0.28],
        guidance_scales=[5.0, 6.0],
    )
    add_textaudio_spec(
        'ddim_entropic', target_nfes=[512], stochastic_enabled=True,
        gamma_targets=[0.15, 0.19, 0.23, 0.27, 0.31, 0.34, 0.37, 0.4],
        guidance_scales=[5.0, 6.0],
    )
    ...
    cfg.logging.project = "cobit_libritts"
